...
function Utility:EnableDragging(Frame)
    ...
    function Library:CreateNotification(Title, Text, Duration)
        ...
        task.spawn(function()
            ...
            if not CoreGui:FindFirstChild('Visual UI Library | Notifications') then
                Utility:Create('ScreenGui', {
                    Name = 'Visual UI Library | Notifications',
                    Parent = CoreGui
                })
            else
                Utility:Create('Frame', {
                    Parent = CoreGui:FindFirstChild('Visual UI Library | Notifications'),
                ...
                Amount = Amount + 1
                local Holder = CoreGui:FindFirstChild('Visual UI Library | Notifications')['Notification'..tostring(Amount)]
...
function Library:SetTransparency(Transparency, NotificationBool)
    local UI = CoreGui:WaitForChild('Visual UI Library')
    local Blacklist = {
        'UIPadding',
        'UICorner',
        'UIStroke',
...
Frame.Size = UDim2.new(0.8, 0, 0.8, 0) 
    ...
    Name = Name or 'Slider'
    MinimumValue = MinimumValue or 1
    MaximumValue = MaximumValue or 100
    SliderColor = SliderColor or Color3.fromRGB(0, 125, 255)
    ...
    if DefaultValue then
        ...
        SliderTrail.Size = UDim2.new((DefaultValue - MinimumValue) / (MaximumValue - MinimumValue), 0, 0, SliderTrail.Size.Y.Offset)
        task.spawn(function()
    ...
    SliderButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            local function UpdateSlider(InputPosition)
                local RelativePosition = math.clamp(InputPosition.X - SliderTrail.AbsolutePosition.X, 0, SliderButton.AbsoluteSize.X)
                SliderTrail.Size = UDim2.new(0, RelativePosition, 0, SliderTrail.Size.Y.Offset)
                CurrentValue = math.floor(MinimumValue + (RelativePosition / SliderButton.AbsoluteSize.X) * (MaximumValue - MinimumValue))
    ...
    function SliderFunctions:Set(Value)
        ...
        SliderNumber.Text = tostring(CurrentValue)
        SliderTrail.Size = UDim2.new((CurrentValue - MinimumValue) / (MaximumValue - MinimumValue), 0, 0, SliderTrail.Size.Y.Offset)
        Callback(CurrentValue)
    end
